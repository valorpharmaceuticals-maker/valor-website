...
export default function HomeContent({ content }: { content: SiteContent }) {
  const { hero, highlights, stats, chairman, vision, mission, products } = content;
  ...
  return (
    <>
      ...
      <section className="section">
        <div className="container">
          <SectionHead center eyebrow={home.whyChooseUs.eyebrow} title={home.whyChooseUs.title} lead={home.whyChooseUs.lead} />
            ...
            {highlights.map((f) => (
              <Col xs={24} sm={12} lg={6} key={f.title}>
                <div className="feature-card">
                  ...
                  <Title level={4} style={{ marginTop: 0 }}>{f.title}</Title>
                  <Paragraph style={{ color: "var(--brand-muted)", marginBottom: 0 }}>{f.description}</Paragraph>
                </div>
              </Col>
            ))}
          </Row>
      ...
      <section className="section section--soft">
        <div className="container">
          <div className="split">
            ...
            <div>
              <span className="eyebrow">{home.chairman.eyebrow}</span>
              ...
              <div style={{ marginTop: 8 }}>
                <strong style={{ display: "block", fontSize: 17 }}>{chairman.name}</strong>
                <span style={{ color: "var(--brand-muted)" }}>{chairman.role}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="section section--soft">
        <div className="container">
          <div style={{ display: "flex", flexWrap: "wrap", gap: 16, alignItems: "flex-end", justifyContent: "space-between" }}>
            <SectionHead eyebrow={home.featuredProducts.eyebrow} title={home.featuredProducts.title} lead={home.featuredProducts.lead} />
            <Link href="/products">
              <Button size="large">{home.featuredProducts.ctaLabel} <RightOutlined /></Button>
            </Link>
          </div>
            ...
            {products.slice(0, 6).map((p) => (
              <Col xs={24} sm={12} lg={8} key={p.name}>
                <Card variant="outlined" style={{ height: "100%" }}>
                  ...
                  <div style={{ color: "var(--brand-primary)", fontWeight: 600, fontSize: 13, marginBottom: 12 }}>{p.form}</div>
          ...
          >
            ...
            <Link href="/contact">
              <Button size="large" style={{ background: "#fff", color: "var(--brand-primary-dark)", fontWeight: 700 }}>
                {home.cta.buttonLabel} <RightOutlined />
              </Button>
            </Link>
          </div>
        </div>
